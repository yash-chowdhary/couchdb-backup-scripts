        ...
        for(let i=0;i<35;i++){
            if(object.rows[i].id.substring(0,1) != '_'){        //filters out ones that don't have nested databases
                if(object.rows[i].doc.hasOwnProperty('cms')){   //filters out the ones that have cms & dragonfly databases
                    ...
                    var cms_cmdCommand_replace = 'replacescript.sh '+cmsBackupFileName;
                    console.log(cms_cmdCommand_replace);
                    cmd.get(cms_cmdCommand_replace,function(){
                        console.log('(cms) replacement complete. ' + cmsBackupFileName);
                    });
                    ...
                    if(i!=3){
                        ...
                        cmd.get(dragonfly_cmdCommand_replace,function(){
                            console.log('(dragonfly) replacement complete. ' + dragonflyBackupFileName);
                        });
                    }
                }   else{
                    ...
                    cmd.get(cmdCommand_replace,function(err,data,stderr){
                        console.log('replacement complete. ' + backupFileName);
                    });
